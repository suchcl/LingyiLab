...
egg.js是在koa的基础上封装的，遵循洋葱模型。

中间件可以对全局生效，也可以设置只对部分模块使用。

egg中的中间件，需要固定的写到app/middleware目录下，中间件文件写好了之后，需要在config/config.default.js中配置一下，告诉egg应用，我写好了一个中间件，在项目中注册一下，项目都可以使用了。
...
这是一个功能简单但完整的中间件，基本上就是固定格式，在实际的中间件开发中，按照这个格式写就可以了，只是其中的功能实现部分，需要根据场景去实现。

next参数一定要有，否则中间件部分的内容执行完成之后，不会向下继续执行，next()就是向下执行的作用
...
中间件的注册，只需要根据约定在config/config.default.js中注册就可以了，不需要单独引入

在config/config.default.js中注册的中间件，全局生效，使用的地方不需要单独引入，如果只想在 某些路由中生效，那么可以注册路由级别的中间件。
