...
> 曾尝试在webpack.config.js中在alias配置项中做路径别名的配置，但是始终尝试不成功，在组件中使用配置的路径别名的时候，提示‘找不到模块“@/pages/NewsList/index”或其相应的类型声明’。

**下面将查询到的配置方式但是结果不成功的方式贴出来**

配置:webpack.config.js中找到alias配置项：
...
![create-react-app项目中配置路径别名失败](./images/i66.png)
