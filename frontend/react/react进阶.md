...
2. 事件通过event.target得到发生事件的DOM对象
   ...
   1. 学习ref的时候，ref的官方的文档告诉我们，不要过渡的使用ref，那么什么时候可以省略ref呢？
      ...
      1. 发生事件的元素，正好是我们要操作获取值的元素，则可以省略ref
         ...
         如这个案例中，我们期望获取input控件的值，同时也是期望在input控件发生onblur事件时做一些事情，那么这个时候input控件的ref就可以省略，可以通过event.target的方式获取到input控件属性
...
2. 非受控组件
   ...
   1. react中，所有的输入类控件，如input、checkbox、radio等，这些控件的值是现用现取，就是非受控组件。
...
> react组件的生命周期众多，不容易记忆，我们可以有选择性的记忆，比较重要的是render和componentDidMount.render，去渲染页面。componentDidMount表示页面已经挂载到了DOM上，在这个阶段，可以做一些数据初始化的事情，如发送网络请求、订阅消息、开启定时器等，总之和页面相关的一些动作，都放在这个阶段中。
...
static getDerivedStateFromError(error){}配合componentDidCatch
...
父子组件：props
兄弟组件：消息订阅/发布、集中式管理
祖孙组件（跨级组件）：消息订阅/发布、集中式管理、conText（开发用的少，插件封装用的多）
```
