...
2. 多道程序执行特点
   ...
   - 每一时刻在CPU上运行的程序只有1个，但宏观上，多道程序实现了并发执行
...
<img src="../images/i27.png" alt="不同进程状态之间的转换" style="zoom:33%;" />
...
1. PCB是进程组成中最关键的部分：每个进程都有唯一的PCB，操作系统根据PCB对进程实施控制和管理；
...
**临界区：**在每个进程中访问临界资源的一段程序称为临界区，简称为CS区

**互斥进程进入临界区的准则：**

1. 若有若干进程要求进入空闲的临界区，一次仅允许一个进程进入；
2. 任何时候，处于临界区的程序不能多于1个；若区内已经有进程进入，则其他进程必须等待；
3. 处于临界区的进程要在有限时间内退出，以便其他进程能及时进入自己的临界区；
4. 若进程不能进入自己的临界区，则应让出CPU，避免进程出现“忙等”现象；
...
1. 原语操作：
   ...
   原语，是机器指令的延伸，往往是为了某些特定的功能而编制的一段系统程序。

2. 软件锁：

###### 2.4.2.2 信号量及P、V操作原语

**信号量：**也叫信号灯，由一个整型变量(信号量值)和指向PCB的指针构成的数据结构

现代操作系统中常用的信号量是由两个成员组成的数据结构，其中，一个成员是整型变量，表示该信号量的值；另一个成员是指向PCB的指针。

**信号量的操作，有一些严格的限制：**

1. 信号量可以赋初值，且初值为非负数。信号量的初值可以由系统根据资源情况和使用需求来确定。在初始条件下，信号量的指针可以置为0，表示队列为空
2. 使用过程中，信号量的值可以修改，但是只能由P操作和V操作来访问，不允许通过其他方式来查看或操作信号量
